...
# ============================================================
# DISPLAY-ONLY COMPOSITE ALERT SYSTEM
# ============================================================
# This overrides the alert_level from the forecast CSV inside the app only.
# It is leakage-safe because it uses:
#   1. model outputs for the forecast row itself,
#   2. previous H+0 model probabilities/cases from earlier origin weeks, and
#   3. actual dengue cases strictly BEFORE the selected origin week.
#
# It does NOT use actual target-week dengue cases for H+1, H+2, H+3, etc.
...
def week_index(year: Any, week: Any) -> int:
    """Sortable epidemiological week index. Uses 60 as a safe year multiplier."""
    try:
        return int(year) * 60 + int(week)
    except Exception:
        return 0


@st.cache_data(show_spinner=False)
def load_historical_actual_cases(path_text: str) -> Optional[pd.DataFrame]:
    """
    Loads actual cases for pre-forecast historical context only.

    Anti-leak rule is applied later:
    for each forecast origin, only rows with week_index < origin_week_index
    are used. This means actual future target-week cases are never used for
    forecast alert coloring.
    """
    path = Path(path_text)
    ...
    required = ["barangay", "year", "week", "dengue_cases"]
    ...
    df["dengue_cases"] = pd.to_numeric(df["dengue_cases"], errors="coerce").fillna(0)

    df = df.dropna(subset=["year", "week"])
    df["year"] = df["year"].astype(int)
    df["week"] = df["week"].astype(int)
    df["week_idx"] = df.apply(lambda row: week_index(row["year"], row["week"]), axis=1)

    df = (
        df.groupby(["barangay", "year", "week", "week_idx"], as_index=False)
        .agg(dengue_cases=("dengue_cases", "sum"))
    )

    return df


def add_previous_h0_model_context(forecast_df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds previous origin-week H+0 model context.

    Example:
    For barangay Guadalupe at origin 2025 W10, this can use the H+0 model
    probabilities from previous origins such as 2025 W9 and 2025 W8.

    This is not leakage because those model outputs belong to earlier origin weeks.
    """
    df = forecast_df.copy()

    needed = ["barangay", "origin_year", "origin_week", "horizon", "predicted_cases", "outbreak_probability"]
    if any(col not in df.columns for col in needed):
        df["prev_h0_probability_1"] = 0.0
        df["prev_h0_probability_2"] = 0.0
        df["prev_h0_predicted_cases_1"] = 0.0
        df["prev_h0_predicted_cases_2"] = 0.0
        return df

    df["origin_idx"] = df.apply(lambda row: week_index(row["origin_year"], row["origin_week"]), axis=1)
    df["horizon"] = pd.to_numeric(df["horizon"], errors="coerce").fillna(0).astype(int)
    df["outbreak_probability"] = pd.to_numeric(df["outbreak_probability"], errors="coerce").fillna(0).clip(0, 1)
    df["predicted_cases"] = pd.to_numeric(df["predicted_cases"], errors="coerce").fillna(0).clip(lower=0)

    h0 = df.loc[df["horizon"] == 0, [
        "barangay", "origin_year", "origin_week", "origin_idx",
        "outbreak_probability", "predicted_cases"
    ]].copy()

    h0 = h0.sort_values(["barangay", "origin_idx"]).copy()

    h0["prev_h0_probability_1"] = h0.groupby("barangay")["outbreak_probability"].shift(1)
    h0["prev_h0_probability_2"] = h0.groupby("barangay")["outbreak_probability"].shift(2)
    h0["prev_h0_predicted_cases_1"] = h0.groupby("barangay")["predicted_cases"].shift(1)
    h0["prev_h0_predicted_cases_2"] = h0.groupby("barangay")["predicted_cases"].shift(2)

    h0_context = h0[[
        "barangay", "origin_year", "origin_week",
        "prev_h0_probability_1", "prev_h0_probability_2",
        "prev_h0_predicted_cases_1", "prev_h0_predicted_cases_2",
    ]].copy()

    df = df.merge(
        h0_context,
        on=["barangay", "origin_year", "origin_week"],
        how="left",
    )
...
def add_recent_actual_case_context(
    forecast_df: pd.DataFrame,
    historical_cases_df: Optional[pd.DataFrame],
) -> pd.DataFrame:
    """
    Adds recent actual dengue case context using only weeks BEFORE the origin week.

    For each forecast row, this computes:
    - recent_actual_cases_1: actual cases from the most recent available past week
    - recent_actual_cases_2: actual cases from the second most recent available past week
    - recent_actual_cases_3: actual cases from the third most recent available past week
    - recent_actual_cases_mean_3
    - recent_actual_cases_max_3
    - recent_actual_cases_trend: recent week compared with the average of weeks 2 and 3

    Important anti-leak rule:
    actual cases are filtered with actual_week_idx < forecast_origin_idx.
    """
    df = forecast_df.copy()

    default_cols = {
        "recent_actual_cases_1": 0.0,
        "recent_actual_cases_2": 0.0,
        "recent_actual_cases_3": 0.0,
        "recent_actual_cases_mean_3": 0.0,
        "recent_actual_cases_max_3": 0.0,
        "recent_actual_cases_trend": 0.0,
    }

    if historical_cases_df is None or historical_cases_df.empty:
        for col, val in default_cols.items():
            df[col] = val
        return df

    if "origin_idx" not in df.columns:
        df["origin_idx"] = df.apply(lambda row: week_index(row["origin_year"], row["origin_week"]), axis=1)

    actual = historical_cases_df.copy()
    actual["week_idx"] = pd.to_numeric(actual["week_idx"], errors="coerce").fillna(0).astype(int)
    actual["dengue_cases"] = pd.to_numeric(actual["dengue_cases"], errors="coerce").fillna(0)
    actual = actual.sort_values(["barangay", "week_idx"]).copy()

    # Build compact lookup lists per barangay. This avoids future leakage by
    # filtering to week_idx < origin_idx for each forecast origin.
    actual_lookup = {
        brgy: group[["week_idx", "dengue_cases"]].to_records(index=False).tolist()
        for brgy, group in actual.groupby("barangay")
    }

    cache: Dict[tuple, tuple] = {}

    def recent_stats(barangay: Any, origin_idx: Any) -> tuple:
        brgy = standardize_barangay_name(barangay)
        oi = int(origin_idx)
        ...
        rows = actual_lookup.get(brgy, [])
        past_cases = [float(cases) for wk, cases in rows if int(wk) < oi]

        last3 = list(reversed(past_cases[-3:]))
        while len(last3) < 3:
            last3.append(0.0)
        ...
        older_mean = (c2 + c3) / 2.0
        if older_mean > 0:
            trend = (c1 - older_mean) / older_mean
        else:
            trend = 1.0 if c1 > 0 else 0.0

        result = (c1, c2, c3, mean3, max3, trend)
        cache[key] = result
        return result

    stats = df.apply(
        lambda row: recent_stats(row.get("barangay", ""), row.get("origin_idx", 0)),
        axis=1,
        result_type="expand",
    )

    stats.columns = [
        "recent_actual_cases_1",
        "recent_actual_cases_2",
        "recent_actual_cases_3",
        "recent_actual_cases_mean_3",
        "recent_actual_cases_max_3",
        "recent_actual_cases_trend",
    ]

    for col in stats.columns:
        df[col] = pd.to_numeric(stats[col], errors="coerce").fillna(0)

    return df


def assign_composite_alert_level(
    probability: Any,
    predicted_cases: Any,
    prev_h0_probability_1: Any = 0,
    prev_h0_probability_2: Any = 0,
    prev_h0_predicted_cases_1: Any = 0,
    recent_actual_cases_1: Any = 0,
    recent_actual_cases_mean_3: Any = 0,
    recent_actual_cases_max_3: Any = 0,
    recent_actual_cases_trend: Any = 0,
) -> str:
    """
    Display-only composite alert.

    Inputs are safe:
    - Current row probability and predicted cases are model outputs.
    - Previous H+0 probabilities/cases are from earlier origin weeks.
    - Recent actual cases are strictly before the selected origin week.

    Actual future target-week cases are never used here.
    """
    prob = safe_float(probability, 0.0)
    pred = safe_float(predicted_cases, 0.0)

    p1 = safe_float(prev_h0_probability_1, 0.0)
    p2 = safe_float(prev_h0_probability_2, 0.0)
    prev_pred1 = safe_float(prev_h0_predicted_cases_1, 0.0)

    recent1 = safe_float(recent_actual_cases_1, 0.0)
    recent_mean3 = safe_float(recent_actual_cases_mean_3, 0.0)
    recent_max3 = safe_float(recent_actual_cases_max_3, 0.0)
    recent_trend = safe_float(recent_actual_cases_trend, 0.0)

    probability_rising = (prob > p1 > p2) or ((prob - p1) >= 0.08 and p1 > 0)
    predicted_cases_rising = pred > prev_pred1 and prev_pred1 > 0

    # Very high: strong current model signal or strong recent burden.
    if (
        prob >= 0.70
        or pred >= 10
        or recent_max3 >= 10
    ...
    # High: high model signal, high predicted burden, or recent actual escalation.
    if (
        prob >= 0.50
        or pred >= 5
        or recent_max3 >= 6
        or (recent1 >= 4 and recent_trend >= 1.00)
        or (prob >= 0.40 and probability_rising)
    ):
        return "High"

    # Moderate: threshold-level probability, moderate predicted burden,
    # or clear pre-forecast upward movement.
    if (
        prob >= 0.35
        or pred >= 3
        or recent_max3 >= 3
        or recent_trend >= 0.50
        or (prob >= 0.25 and probability_rising)
        or (predicted_cases_rising and pred >= 2)
    ):
        return "Moderate"

    # Watch: near-threshold probability, nonzero predicted cases,
    # nonzero recent actual cases, or a rising probability signal.
    if (
        prob >= 0.20
        or pred >= 1
        or recent_mean3 > 0
        or probability_rising
    ):
        return "Watch"
...
def apply_display_alert_levels(
    forecast_df: pd.DataFrame,
    historical_cases_df: Optional[pd.DataFrame] = None,
    scope: str = "barangay",
) -> pd.DataFrame:
    """
    Recalculates alert levels inside app.py without changing the R scripts.

    The original CSV alert is preserved as alert_level_original.
    The app uses the recalculated display alert as alert_level.
    """
    df = forecast_df.copy()

    if df.empty:
        df["alert_level_original"] = "Not available"
        df["display_alert_level"] = "Low"
        df["alert_level"] = "Low"
        return df

    if "alert_level" in df.columns:
        df["alert_level_original"] = df["alert_level"].astype(str)
    else:
        df["alert_level_original"] = "Not available"

    df["origin_year"] = pd.to_numeric(df.get("origin_year", 0), errors="coerce").fillna(0).astype(int)
    df["origin_week"] = pd.to_numeric(df.get("origin_week", 0), errors="coerce").fillna(0).astype(int)
    df["horizon"] = pd.to_numeric(df.get("horizon", 0), errors="coerce").fillna(0).astype(int)
    df["predicted_cases"] = pd.to_numeric(df.get("predicted_cases", 0), errors="coerce").fillna(0).clip(lower=0)
    df["outbreak_probability"] = pd.to_numeric(df.get("outbreak_probability", 0), errors="coerce").fillna(0).clip(0, 1)
    df["origin_idx"] = df.apply(lambda row: week_index(row["origin_year"], row["origin_week"]), axis=1)

    if scope == "barangay" and "barangay" in df.columns:
        df["barangay"] = df["barangay"].apply(standardize_barangay_name)
        df = add_previous_h0_model_context(df)
        df = add_recent_actual_case_context(df, historical_cases_df)
    else:
        # City-level fallback: no barangay-specific historical context.
        for col in [
            "prev_h0_probability_1", "prev_h0_probability_2",
            "prev_h0_predicted_cases_1", "recent_actual_cases_1",
            "recent_actual_cases_mean_3", "recent_actual_cases_max_3",
            "recent_actual_cases_trend",
        ]:
            if col not in df.columns:
                df[col] = 0.0

    df["display_alert_level"] = df.apply(
        lambda row: assign_composite_alert_level(
            probability=row.get("outbreak_probability", 0),
            predicted_cases=row.get("predicted_cases", 0),
            prev_h0_probability_1=row.get("prev_h0_probability_1", 0),
            prev_h0_probability_2=row.get("prev_h0_probability_2", 0),
            prev_h0_predicted_cases_1=row.get("prev_h0_predicted_cases_1", 0),
            recent_actual_cases_1=row.get("recent_actual_cases_1", 0),
            recent_actual_cases_mean_3=row.get("recent_actual_cases_mean_3", 0),
            recent_actual_cases_max_3=row.get("recent_actual_cases_max_3", 0),
            recent_actual_cases_trend=row.get("recent_actual_cases_trend", 0),
        ),
        axis=1,
    )

    df["alert_level"] = df["display_alert_level"]
    ...
    st.markdown(
        ...
        <div class="legend-box">
            ...
            <div class="small-note" style="margin-top:.55rem;">
                Colors use a leakage-safe composite display alert based on model outputs,
                previous origin-week model signals, and actual cases before the selected origin week only.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def create_barangay_prediction_map(shape_gdf: gpd.GeoDataFrame, barangay_rows: pd.DataFrame) -> folium.Map:
    ...
    tooltip = folium.GeoJsonTooltip(
        ...
        aliases=["Barangay:", "Predicted case range:", "Outbreak probability:", "Composite alert level:"],
        sticky=True,
        style=(
    ...
    popup = folium.GeoJsonPopup(
        ...
        aliases=[
            ...
            "Range based on:", "Outbreak probability:", "Composite alert level:", "Recommended interventions:",
        ],
        localize=True,
        labels=True,
        max_width=430,
...
historical_cases_df = load_historical_actual_cases(str(DATASET_PATH))
...
# Recalculate display alert levels inside the app only.
# This preserves the original CSV alert as alert_level_original, then replaces
# alert_level with a leakage-safe composite display alert for the map/table.
barangay_df = apply_display_alert_levels(
    barangay_df,
    historical_cases_df=historical_cases_df,
    scope="barangay",
)
city_df = apply_display_alert_levels(
    city_df,
    historical_cases_df=None,
    scope="city",
)
...
summary_table = summary_table.rename(columns={
    ...
    "alert_level": "Composite alert level/status",
    "range_basis": "Range basis",
})[["Forecast horizon", "Target year", "Target week", "Citywide case range", "Outbreak probability", "Composite alert level/status", "Range basis"]]
...
for tab, horizon in zip(tabs, DISPLAY_HORIZONS):
    with tab:
        ...
        with c3:
            st.metric("Composite alert level/status", str(city_row["alert_level"]))
        ...
        city_table = pd.DataFrame({
            ...
            "Composite alert level/status": [str(city_row["alert_level"])],
        })
        ...
        table = table.rename(columns={
            ...
            "range_basis": "Range basis",
            "probability_display": "Outbreak probability",
            "alert_level": "Composite alert level",
        })
